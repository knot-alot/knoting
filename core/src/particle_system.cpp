...
inline uint32_t toAbgr(const float* _rgba) {
    return 0 | (uint8_t(_rgba[0] * 255.0f) << 0) | (uint8_t(_rgba[1] * 255.0f) << 8) |
           (uint8_t(_rgba[2] * 255.0f) << 16) | (uint8_t(_rgba[3] * 255.0f) << 24);
}

inline uint32_t toAbgr(float _rr, float _gg, float _bb, float _aa) {
    return 0 | (uint8_t(_rr * 255.0f) << 0) | (uint8_t(_gg * 255.0f) << 8) | (uint8_t(_bb * 255.0f) << 16) |
...
struct Emitter {
    ...
    void spawn(float _dt) {
        float mtx[16];
        bx::mtxSRT(mtx, 1.0f, 1.0f, 1.0f, m_uniforms.m_angle[0], m_uniforms.m_angle[1], m_uniforms.m_angle[2],
        ...
        for (uint32_t ii = 0; ii < numParticles && m_num < m_max; ++ii) {
            ...
            m_max = m_uniforms.m_maxParticles;

            const float startOffset =
                bx::lerp(m_uniforms.m_offsetStart[0], m_uniforms.m_offsetStart[1], bx::frnd(&m_rng));
    ...
    uint32_t render(const float _uv[4],
...
struct ParticleSystem {
    void init(uint16_t _maxEmitters, bx::AllocatorI* _allocator) {
        ...
        m_texture =
            bgfx::createTexture2D(SPRITE_TEXTURE_SIZE, SPRITE_TEXTURE_SIZE, false, 1, bgfx::TextureFormat::RGBA8);
    ...
    EmitterSpriteHandle createSprite(uint16_t _width, uint16_t _height, const void* _data, int channels) {
        ...
        if (isValid(handle)) {
            ...
            bgfx::updateTexture2D(m_texture, 0, 0, pack.m_x, pack.m_y, pack.m_width, pack.m_height,
                                  bgfx::copy(_data, pack.m_width * pack.m_height * channels));
        }

        return handle;
    }

    void destroy(EmitterSpriteHandle _handle) { m_sprite.destroy(_handle); }
    ...
    void render(uint8_t _view, const float* _mtxView, const bx::Vec3& _eye) {
        if (0 != m_num) {
            ...
            const uint32_t numVertices =
                bgfx::getAvailTransientVertexBuffer(m_num * 4, PosColorTexCoord0Vertex::ms_layout);
    ...
    knot::components::ShaderProgram m_shader;
...
EmitterSpriteHandle psCreateSprite(uint16_t _width, uint16_t _height, const void* _data, int channels) {
    return s_ctx.createSprite(_width, _height, _data, channels);
}
