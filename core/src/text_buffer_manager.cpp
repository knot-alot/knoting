...
#include "../bgfx/bgfx/examples/common/font/fs_font_distance_field_drop_shadow.bin.h"
#include "../bgfx/bgfx/examples/common/font/fs_font_distance_field_drop_shadow_image.bin.h"
#include "../bgfx/bgfx/examples/common/font/fs_font_distance_field_outline.bin.h"
#include "../bgfx/bgfx/examples/common/font/fs_font_distance_field_outline_drop_shadow_image.bin.h"
#include "../bgfx/bgfx/examples/common/font/fs_font_distance_field_outline_image.bin.h"
#include "../bgfx/bgfx/examples/common/font/fs_font_distance_field_subpixel.bin.h"
#include "../bgfx/bgfx/examples/common/font/vs_font_basic.bin.h"
#include "../bgfx/bgfx/examples/common/font/vs_font_distance_field.bin.h"
#include "../bgfx/bgfx/examples/common/font/vs_font_distance_field_drop_shadow.bin.h"
#include "../bgfx/bgfx/examples/common/font/vs_font_distance_field_drop_shadow_image.bin.h"
#include "../bgfx/bgfx/examples/common/font/vs_font_distance_field_outline.bin.h"
#include "../bgfx/bgfx/examples/common/font/vs_font_distance_field_outline_drop_shadow_image.bin.h"
#include "../bgfx/bgfx/examples/common/font/vs_font_distance_field_outline_image.bin.h"
#include "../bgfx/bgfx/examples/common/font/vs_font_distance_field_subpixel.bin.h"
...
class TextBuffer {
   ...
   private:
    ...
    static uint32_t toABGR(uint32_t _rgba) {
        return (((_rgba >> 0) & 0xff) << 24) | (((_rgba >> 8) & 0xff) << 16) | (((_rgba >> 16) & 0xff) << 8) |
               (((_rgba >> 24) & 0xff) << 0);
    }
    ...
    void setOutlineColor(uint32_t _i, uint32_t _rgbaOutline) { m_vertexBuffer[_i].rgbaOutline = _rgbaOutline; }
...
void TextBuffer::appendGlyph(FontHandle _handle, CodePoint _codePoint, bool shadow) {
    ...
    if (font.ascender > m_lineAscender || (font.descender < m_lineDescender)) {
        ...
        m_lineAscender = font.ascender;
        m_lineGap = font.lineGap;
        verticalCenterLastLine((txtDecals), (m_penY - m_lineAscender),
    ...
    if (shadow) {
        if (atlasRegion.getType() != AtlasRegion::TYPE_BGRA8) {
            ...
            atlas->packUV(glyph->regionIndex, (uint8_t*)m_vertexBuffer,
                          sizeof(TextVertex) * m_vertexCount + offsetof(TextVertex, u2), sizeof(TextVertex));
    ...
    if (m_styleFlags & STYLE_BACKGROUND && m_backgroundColor & 0xff000000) {
        ...
        atlas->packUV(blackGlyph.regionIndex, (uint8_t*)m_vertexBuffer,
                      sizeof(TextVertex) * m_vertexCount + offsetof(TextVertex, u), sizeof(TextVertex));
    ...
    if (!shadow && atlasRegion.getType() == AtlasRegion::TYPE_BGRA8) {
        ...
        atlas->packUV(glyph->regionIndex, (uint8_t*)m_vertexBuffer,
                      sizeof(TextVertex) * m_vertexCount + offsetof(TextVertex, u1), sizeof(TextVertex));
...
TextBufferManager::TextBufferManager(FontManager* _fontManager) : m_fontManager(_fontManager) {
    ...
    m_basicProgram = bgfx::createProgram(bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_font_basic"),
                                         bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_font_basic"), true);

    m_distanceProgram =
        bgfx::createProgram(bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_font_distance_field"),
    ...
    m_distanceSubpixelProgram = bgfx::createProgram(
        ...
        bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_font_distance_field_subpixel"), true);
    ...
    m_distanceDropShadowProgram = bgfx::createProgram(
        ...
        bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_font_distance_field_drop_shadow"), true);
    ...
    m_distanceDropShadowImageProgram = bgfx::createProgram(
        ...
        bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_font_distance_field_drop_shadow_image"), true);
    ...
    m_distanceOutlineProgram = bgfx::createProgram(
        ...
        bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_font_distance_field_outline"), true);
    ...
    m_distanceOutlineImageProgram = bgfx::createProgram(
        ...
        bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_font_distance_field_outline_image"), true);
    ...
    m_distanceOutlineDropShadowImageProgram = bgfx::createProgram(
        ...
        bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_font_distance_field_outline_drop_shadow_image"), true);
...
void TextBufferManager::submitTextBuffer(TextBufferHandle _handle, bgfx::ViewId _id, int32_t _depth) {
    ...
    switch (bc.fontType) {
        ...
        case FONT_TYPE_DISTANCE_OUTLINE: {
            ...
            float params[4] = {0.0f, (float)m_fontManager->getAtlas()->getTextureSize() / 512.0f, 0.0f,
                               bc.textBuffer->getOutlineWidth()};
            bgfx::setUniform(u_params, &params);
            break;
        }
        ...
        case FONT_TYPE_DISTANCE_DROP_SHADOW: {
            ...
            float dropShadowColorVec[4] = {((dropShadowColor >> 16) & 0xff) / 255.0f,
                                           ((dropShadowColor >> 8) & 0xff) / 255.0f, (dropShadowColor & 0xff) / 255.0f,
                                           (dropShadowColor >> 24) / 255.0f};
            bgfx::setUniform(u_dropShadowColor, &dropShadowColorVec);

            float params[4] = {0.0f, (float)m_fontManager->getAtlas()->getTextureSize() / 512.0f,
                               bc.textBuffer->getDropShadowSoftener(), 0.0};
            bgfx::setUniform(u_params, &params);
            break;
        }
        ...
        case FONT_TYPE_DISTANCE_OUTLINE_DROP_SHADOW_IMAGE: {
            ...
            float params[4] = {0.0f, (float)m_fontManager->getAtlas()->getTextureSize() / 512.0f,
                               bc.textBuffer->getDropShadowSoftener(), bc.textBuffer->getOutlineWidth()};
            bgfx::setUniform(u_params, &params);
            break;
        }
    }
    ...
    switch (bc.bufferType) {
        case BufferType::Static: {
            ...
            if (bgfx::kInvalidHandle == bc.vertexBufferHandleIdx) {
                ...
                vbh =
                    bgfx::createVertexBuffer(bgfx::copy(bc.textBuffer->getVertexBuffer(), vertexSize), m_vertexLayout);
