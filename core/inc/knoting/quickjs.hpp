// Licensed under CC0. See https://github.com/ftk/quickjspp for more information.
// Modified by Knot-Alot team, to work with QuickJSPP, and be compatible with MSVC, and Orbis.
 ...
 * Describes how to convert type R to/from JSValue. Second template argument can be used for SFINAE/enable_if type
 * filters.
 */
...
struct js_traits<
    Int,
    std::enable_if_t<std::is_integral_v<Int> && sizeof(Int) <= sizeof(int64_t) && !std::is_same_v<Int, uint64_t>>> {
...
struct js_traits<float> {
    ...
    static JSValue wrap(JSContext* ctx, float i) noexcept { return JS_NewFloat64(ctx, i); }
};
        ...
        switch (tag) {
            ...
            case JS_TAG_OBJECT:
                ...
                JS_ThrowTypeError(ctx, "Expected type %s, got object with classid %d",
                                  QJSPP_TYPENAME(std::variant<Ts...>), JS_GetClassID(v, nullptr));
                break;
...
std::tuple<std::decay_t<Args>...> unwrap_args(JSContext* ctx, int argc, JSValueConst* argv) {
    return unwrap_args_impl<std::tuple<std::decay_t<Args>...>>(ctx, argc, argv,
...
JSValue wrap_call(JSContext* ctx, Callable&& f, int argc, JSValueConst* argv) noexcept {
    if constexpr (std::is_same_v<R, void>) {
        ...
        return js_traits<std::decay_t<R>>::wrap(
            ctx, std::apply(std::forward<Callable>(f), unwrap_args<Args...>(ctx, argc, argv)));
    }
}
...
JSValue wrap_this_call(JSContext* ctx, Callable&& f, JSValueConst this_value, int argc, JSValueConst* argv) noexcept {
    if constexpr (std::is_same_v<R, void>) {
        ...
        return js_traits<std::decay_t<R>>::wrap(
            ctx, std::apply(std::forward<Callable>(f), std::tuple_cat(unwrap_args<FirstArg>(ctx, 1, &this_value),
...
struct js_traits<std::shared_ptr<T>> {
    ...
    static void registerDerivedClass(JSClassID derived_class_id, ptr_cast_fcn_t ptr_cast_fcn) {
        ...
        js_traits<std::shared_ptr<D>>::registerWithBase = [old_registerWithBase = std::move(old_registerWithBase)](
            ...
            registerDerivedClass<D>(
                derived_class_id, [derived_cast_fcn = std::move(derived_ptr_cast_fcn)](JSContext* ctx, JSValueConst v) {
    ...
    template <typename B>
    static std::enable_if_t<std::is_same_v<B, T> || std::is_same_v<B, void>> ensureCanCastToBase() {}
...
struct js_traits<detail::function> {
    ...
    static void register_class(JSContext* ctx, const char* name) {
        ...
        JSClassDef def{name,
                       ...
                       nullptr,  // mark
                       // call
                       [](JSContext* ctx, JSValueConst func_obj, JSValueConst this_val, int argc, JSValueConst* argv,
                          int flags) -> JSValue {
 ...
 * A JSValue can be released by either JSValue x = std::move(value); or JSValue x = value.release(), then the Value
 * becomes invalid and FreeValue won't be called Can be converted to C++ type, for example: auto string =
 * value.as<std::string>() Properties can be accessed (read/write): value["property1"] = 1; value[2] = "2";
 */
class Value {
   public:
    JSValue v;
   ...
   public:
    ...
    Value& add(const char* name, Function&& f) {
        (*this)[name] =
            js_traits<decltype(std::function{std::forward<Function>(f)})>::wrap(ctx, std::forward<Function>(f));
        return *this;
    }
    ...
    // add<&T::f>("f");
    template <auto F>
    std::enable_if_t<!std::is_member_object_pointer_v<decltype(F)>, Value&> add(const char* name) {
    ...
    Value& add_getter(const char* name) {
        auto prop = JS_NewAtom(ctx, name);
        using fgetter = fwrapper<FGet, true>;
        int ret = JS_DefinePropertyGetSet(ctx, v, prop, js_traits<fgetter>::wrap(ctx, fgetter{name}), JS_UNDEFINED,
    ...
    // add<&T::member>("member");
    template <auto M>
    std::enable_if_t<std::is_member_object_pointer_v<decltype(M)>, Value&> add(const char* name) {
...
class Runtime {
   public:
    ...
    Runtime() {
        ...
        js_std_init_handlers(rt);  // TODO: Implement our own stuff
...
class Context {
    ...
    class Module {
       ...
       public:
        ...
        Module& add(const char* name, T&& value) {
            return add(name, js_traits<std::decay_t<T>>::wrap(ctx, std::forward<std::decay_t<T>>(value)));
        }
        ...
        JSModuleDef* get_module() const { return m; }
        ...
        class class_registrar {
           ...
           public:
            ...
            class_registrar& constructor(const char* name = nullptr) {
                if (!name)
                    name = this->name;
                Value ctor = context.newValue(qjs::CTorWrapper<T, Args...>{name});
   ...
   private:
    void init() {
        ...
        js_std_add_helpers(ctx, 0, nullptr);  // TODO: Implement our own
   ...
   public:
    ...
    void dumpException() {
        ...
        JSValue err_file = JS_GetPropertyStr(ctx, exception, "fileName");
...
template <class T, typename... Args>
struct js_traits<CTorWrapper<T, Args...>> {
    static JSValue wrap(JSContext* ctx, CTorWrapper<T, Args...> cw) noexcept {
        return JS_NewCFunction2(
            ...
            [](JSContext* ctx, JSValueConst this_value, int argc, JSValueConst* argv) noexcept -> JSValue {
                ...
                std::shared_ptr<T> ptr =
                    std::apply(std::make_shared<T, Args...>, detail::unwrap_args<Args...>(ctx, argc, argv));
...
/** Convert to/from std::function. Actually accepts/returns callable object that is compatible with function<R
 * (Args...)>.
    ...
    static auto unwrap(JSContext* ctx, JSValueConst fun_obj) {
        ...
        if constexpr (argc == 0) {
            ...
            return [jsfun_obj = Value{ctx, JS_DupValue(ctx, fun_obj)}](Args&&... args) -> R {
                ...
                JSValue result =
                    JS_Call(jsfun_obj.ctx, jsfun_obj.v, JS_UNDEFINED, argc, const_cast<JSValueConst*>(argv));
...
struct js_traits<Function, std::enable_if_t<detail::is_callable_v<Function>>> {
    ...
    static JSValue wrap(JSContext* ctx, Functor&& functor) {
        return js_traits<decltype(std::function{std::declval<Function>()}), int>::wrap(ctx,
                                                                                       std::forward<Functor>(functor));
    }
};

// Convert from std::vector<T> to Array and vice-versa.
template <class T>
struct js_traits<std::vector<T>> {
    ...
    static std::vector<T> unwrap(JSContext* ctx, JSValueConst jsarr) {
        ...
        arr.resize((uint32_t)len);
        for (uint32_t i = 0; i < (uint32_t)len; i++)
            arr[i] = static_cast<T>(jsarray[i]);
        return arr;
    }
};

template <>
struct js_traits<knot::GameObject> {
    static knot::GameObject unwrap(JSContext* ctx, JSValueConst jsval) {
        if (!JS_IsString(jsval))
            JS_ThrowTypeError(ctx, "js_traits<knot::GameObject>::unwrap expects string");

        auto str = JS_ToCString(ctx, jsval);
        knot::uuid uuid;
        if (str) {
        ...
        if (!sceneOpt)
            JS_ThrowTypeError(ctx, "js_traits<knot::GameObject>::unwrap expects valid scene");

        auto& scene = sceneOpt.value().get();
        auto goOpt = scene.get_game_object_from_id(uuid);
        if (!goOpt)
            JS_ThrowTypeError(ctx, "js_traits<knot::GameObject>::unwrap expects valid game object");

        return goOpt.value();
    }

    static JSValue wrap(JSContext* ctx, const knot::GameObject& go) {
        return JS_NewString(ctx, to_string(go.get_id()).c_str());
    }
};

template <>
struct js_traits<knot::uuid> {
    static knot::uuid unwrap(JSContext* ctx, JSValueConst jsval) {
        if (!JS_IsString(jsval))
            JS_ThrowTypeError(ctx, "js_traits<knot::uuid>::unwrap expects string");
    ...
    static JSValue wrap(JSContext* ctx, const knot::uuid& uuid) { return JS_NewString(ctx, to_string(uuid).c_str()); }
};

template <>
struct js_traits<knot::vec2> {
    static knot::vec2 unwrap(JSContext* ctx, JSValueConst v) {
        int e = JS_IsArray(ctx, v);
        if (e == 0)
            JS_ThrowTypeError(ctx, "js_traits<std::vector<T>>::unwrap expects array");
        if (e <= 0)
            return {};
        ...
        auto len = static_cast<int32_t>(jsarray["length"]);
        if (len < 2) {
            JS_ThrowTypeError(ctx, "js_traits<knot::vec2>::unwrap expects array of length 2");
            return {};
        }
...
template <>
struct js_traits<knot::vec3> {
    static knot::vec3 unwrap(JSContext* ctx, JSValueConst v) {
        int e = JS_IsArray(ctx, v);
        if (e == 0)
            JS_ThrowTypeError(ctx, "js_traits<std::vector<T>>::unwrap expects array");
        if (e <= 0)
        ...
        auto len = static_cast<int32_t>(jsarray["length"]);
        if (len < 3) {
            JS_ThrowTypeError(ctx, "js_traits<knot::vec3>::unwrap expects array of length 2");
            return {};
        }
...
template <>
struct js_traits<knot::vec4> {
    static knot::vec4 unwrap(JSContext* ctx, JSValueConst v) {
        int e = JS_IsArray(ctx, v);
        if (e == 0)
            JS_ThrowTypeError(ctx, "js_traits<std::vector<T>>::unwrap expects array");
        if (e <= 0)
        ...
        auto len = static_cast<int32_t>(jsarray["length"]);
        if (len < 4) {
            JS_ThrowTypeError(ctx, "js_traits<knot::vec4>::unwrap expects array of length 2");
            return {};
        }
        ...
        ret.w = static_cast<float>(jsarray[3]);
        return ret;
    }
...
template <>
struct js_traits<knot::quat> {
    static knot::quat unwrap(JSContext* ctx, JSValueConst v) {
        int e = JS_IsArray(ctx, v);
        if (e == 0)
            JS_ThrowTypeError(ctx, "js_traits<std::vector<T>>::unwrap expects array");
        if (e <= 0)
        ...
        auto len = static_cast<int32_t>(jsarray["length"]);
        if (len < 4) {
            JS_ThrowTypeError(ctx, "js_traits<knot::quat>::unwrap expects array of length 2");
            return {};
        }

        ret.w = static_cast<float>(jsarray[0]);
        ret.x = static_cast<float>(jsarray[1]);
        ret.y = static_cast<float>(jsarray[2]);
        ret.z = static_cast<float>(jsarray[3]);
        return ret;
    }
