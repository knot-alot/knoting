...
void WidgetSubsystem::imguiInit( GLFWwindow* window )
{
    ...
    log::info("imgui start vtx layout");
    ...
    log::info("imgui start font layout");
    // Create font
    io.Fonts->AddFontDefault();
    io.Fonts->GetTexDataAsRGBA32( &data, &width, &height );
    log::info("imgui start create tex2d");
    ...
    log::info("imgui start create tex uniform");
    imguiFontUniform = bgfx::createUniform( "s_tex", bgfx::UniformType::Sampler );

    log::info("imgui start create vs shader");
    // Create shader program
//    shader_test.load_shader("imgui", "fs_ocornut_imgui.bin","vs_ocornut_imgui.bin");
//
//    if(shader_test.get_program().idx != bgfx::kInvalidHandle){
//        log::info("found valid shader path using shaderProgram");
//    }
...
//    bgfx::ShaderHandle vs = bgfx::createShader( bgfx::makeRef( vs_ocornut_imgui(), vs_ocornut_imgui_len() ) );
//    log::info("imgui start create fs shader");
//    bgfx::ShaderHandle fs = bgfx::createShader( bgfx::makeRef( fs_ocornut_imgui(), fs_ocornut_imgui_len() ) );
//    imguiProgram = shader_test.get_program();
    ...
    log::info("IDX  , fonttex {} , fontUni {} , program {}", imguiFontTexture.idx,imguiFontUniform.idx, imguiProgram.idx );
}
